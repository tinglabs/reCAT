...
bestEnsembleComplexTSP <- function(test_exp, stageIdxSmp = NULL, TSPFold = 2, beginNum = 10, endNum = 198, output = TRUE, debug = FALSE, name = "", threads = 1, clustMethod = "Corr", base_cycle_range = c(6:9), step_size = 2, max_loop = NULL, max_num = 300)
{
	if (debug == TRUE)
		message("Task Begin! ")
	require(doParallel)
  ...
  #print(c - b)
	...
	else
	{
	  ...
	  print("make cluster")
		...
		print("finished")
		#resultLst <- foreach(clust_num_EM = (beginNum + 1): endNum, .combine='rbind') %dopar% #, .export = c("highCorFind", "forceTSP", "correlation_distance", "normalizeResult", "Mclust", "mclustBIC")
